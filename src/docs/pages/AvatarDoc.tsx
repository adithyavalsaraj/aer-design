  ...
  const overview = (
    <div className="space-y-12">
      ...
      >
        ...
        <CodeBlock
          ts={`<div className="flex items-center gap-3 p-2 hover:bg-aer-muted/20 rounded-lg group">\n  <Avatar src={user.img} status={user.status} />\n  <div className="flex flex-col">\n    <span className="text-sm font-semibold">{user.name}</span>\n    <span className="text-xs text-aer-muted-foreground">{user.role}</span>\n  </div>\n  <Check className="ml-auto opacity-0 group-hover:opacity-100" />\n</div>`}
